...
const app = express();
// Preserve the public HTTPS resource URL behind Railway and similar one-hop
// TLS-terminating proxies. This affects challenge metadata, never auth.
...
// trustProxy preserves the public HTTPS resource URL behind one-hop hosts
// such as Railway. This affects challenge metadata, never auth.
...
function nextLayoutJs(): string {
  return `import { createElement } from 'react';

export const metadata = {
  title: 'x402 seller',
  description: 'Account-free x402 seller generated by paygate init',
};
...
function nextPageJs(opts: TemplateOptions): string {
  return `import { createElement } from 'react';
...
export default function Home() {
  return createElement(
    'main',
    null,
    createElement('h1', null, 'x402 seller'),
    createElement('p', null, 'Health: ', createElement('a', { href: '/api/health' }, '/api/health')),
    createElement('p', null, 'Paid route: ', createElement('code', null, '${opts.method} ${opts.route}')),
  );
}
`;
}
...
`
    : ""
}    output=OutputConfig(example={"ok": True, "data": "paid content"}),
)
# x402 2.17 validates route declarations before its runtime enrichment hook.
discovery_extension["bazaar"]["info"]["input"]["method"] = "${opts.method}"
  ...
  switch (opts.framework) {
    ...
    case "next": {
      ...
      return [
        ...
        { path: "app/layout.js", content: nextLayoutJs() },
        { path: "app/page.js", content: nextPageJs(opts) },
        ...
        {
          path: "README.md",
          content: `# ${opts.projectName}\n\nAccount-free x402 seller (Next.js App Router) generated by\n\`npx paygate init\`. No ArisPay account or API key — the wallet in\n\`PAY_TO_ADDRESS\` receives funds directly. This is a complete standalone\nNext.js project; its paid route handler is wrapped with \`withX402\` from\n\`@x402/next\` (verified against @x402/next 2.20.0), which settles only after\nyour handler succeeds — failed responses are never charged.\n\n\`\`\`bash\nnpm install\ncp .env.example .env.local   # set PAY_TO_ADDRESS\nnpm run dev\n\`\`\`\n\n${readmeCommon(opts, "npm run dev", 3000)}`,
        },
      ];
    }
    case "fastapi":
      return [
        { path: "main.py", content: fastapiMainPy(opts) },
        {
          ...
          content: `# x402 pip package (FastAPI middleware, EVM mechanism, and Bazaar validation), verified against 2.17.0\nx402[fastapi,evm,extensions]>=2.17.0\nfastapi>=0.115.0\nuvicorn>=0.30.0\n`,
        },
        { path: ".env.example", content: envExample(opts) },
        { path: ".gitignore", content: GITIGNORE },
        {
